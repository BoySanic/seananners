...
__device__ static bool match(int64_t seed){
    ...
    for(spawn.z = 0; (!spawnBox.intersectsWith(guessBox) && count <= 200) && !(spawn.z > guessBox.end.z || spawn.x > guessBox.end.x); spawn.z += nextInt(&structureSeed, 64) - nextInt(&structureSeed, 64))
    {
